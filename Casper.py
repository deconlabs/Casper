import math
class Casper:
    def __init__(self, totalD, h, baseInterestFactor, basePenaltyFactor, baseDepositDependence, attackDuration, exponentialTerm=0):
        ...
        self.D = totalD
        # h is the deposit fraction of the validator who is being 'censored'. During a censorship attack self validator represents the victims, during a minority fork those who are voting on the other chain
        self.h = h
    ...
    def processEpoch(self,):
        ...
        for i in range(0, self.numValidators):
            ...
            if(self.totalVoteUnscaled/self.getTotalDepositsUnscaled() > 2/3 and self.lastJustifiedEpoch < self.epoch):
    ...
    def processEpochs(self,n):
        ...
        for i in range(0, n):
            ...
            if(i % (math.ceil(n/1000)) == 0):
                depsDepn = math.pow(
                    self.numValidators*self.validatorInitDeposits[z]*self.INITIAL_SCALE_FACTOR, self.baseDepositDependence)
                interest = math.pow(
